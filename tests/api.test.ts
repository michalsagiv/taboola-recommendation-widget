...
import {
  buildApiUrl,
  getTargetFromOrigin,
  mapResponseToRecommendations,
  normalizeRecommendation,
  pickThumbnail,
} from "../src/api";
import { RecommendationResponseEnvelope } from "../src/types";
...
describe("normalizeRecommendation", () => {
  ...
  it("maps fields and derives target", () => {
    ...
    expect(item?.thumbnailUrl).toBe("https://img.com/a.jpg");
  });
});

describe("mapResponseToRecommendations", () => {
  it("deduplicates and filters invalid entries", () => {
    const payload: RecommendationResponseEnvelope = {
      list: [
